...
# buildkit-file-id: 266a413b-c3b5-4ec5-874f-1c27fa74fc96
...
# and matches the resolved active FEATURE_DIR (so /buildkit.plan can skip git branch pattern checks).
...
resolve_template() {
    ...
    if [ -d "$presets_dir" ]; then
        ...
        if [ -f "$registry_file" ] && command -v python3 >/dev/null 2>&1; then
            ...
            if sorted_presets=$(BUILDKIT_REGISTRY="$registry_file" python3 -c "
import json, sys, os
try:
    with open(os.environ['BUILDKIT_REGISTRY']) as f:
        data = json.load(f)
                ...
                if [ -n "$sorted_presets" ]; then
                    ...
                    while IFS= read -r preset_id; do
                        ...
                        if [ -f "$manifest" ] && command -v python3 >/dev/null 2>&1; then
                            ...
                            local py_stderr
                            py_stderr=$(mktemp)
                            result=$(BUILDKIT_MANIFEST="$manifest" BUILDKIT_TMPL="$template_name" python3 -c "
import sys, os
try:
    import yaml
except ImportError:
    ...
    sys.exit(0)
try:
    with open(os.environ['BUILDKIT_MANIFEST']) as f:
        data = yaml.safe_load(f)
    for t in data.get('provides', {}).get('templates', []):
        if t.get('name') == os.environ['BUILDKIT_TMPL'] and t.get('type', 'template') == 'template':
