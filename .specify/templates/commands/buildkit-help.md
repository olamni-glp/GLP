...
argument-hint: "[a situation, e.g. 'I have three feature branches open — what order do I build them in?'] | info <tool> | list | reconcile"
compatibility: "The deterministic inventory/drill-down works in any terminal (no model, no catalog). Custom-workflow persistence/enforcement/export requires the buildkit PGlite catalog (like /bk-roadmap, /bk-backlog, /bk-plan-order)."
metadata:
  author: "buildkit"
