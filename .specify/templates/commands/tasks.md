...
handoffs: 
  - label: Analyze For Consistency
    agent: buildkit.analyze
    prompt: Run a project analysis for consistency
    send: true
  - label: Implement Project
    agent: buildkit.implement
...
Run `python -m buildkit_cli.pipeline.sidecar start tasks` from the project root.
...
After the sidecar pre-check, run `python -m buildkit_cli.refine resolve tasks`
from the project root. It prints one JSON line
`{"guidance_version_id":…,"stage":"tasks","source":"active|baseline","text_path":"<abs path>"}`
...
and prepend its contents to your task-generation context as additional
guidance.
...
`python -m buildkit_cli.pipeline.sidecar complete tasks` from the project root.
On any abort or error before this point, instead run
`python -m buildkit_cli.pipeline.sidecar fail tasks --error "<one-line summary>"`.
...
the sidecar post-record, run `python -m buildkit_cli.refine record tasks
...
confirm / update / decline. This step NEVER blocks stage completion (SC-003). You may also
size individual user stories/tasks here (`buildkit-size set user_story <id> …` /
`set task <id> …`); see `/bk-size`.
1. Run `buildkit-size prompt tasks --feature <feature-id> --json` (read-only; always exits 0,
...
3. Record the chosen response (advisory — ignore any failure):
   - Confirm → `buildkit-size confirm feature <feature-id> --stage tasks`
   - Update  → `buildkit-size set feature <feature-id> --label <bucket> --stage tasks`
               (add `--points <n>` for a custom value; keep `--label` to retain the bucket name)
   - Decline → `buildkit-size decline feature <feature-id> --stage tasks`
...
- `buildkit-size tokens record tasks --feature <feature-id> --total <N> --method self-reported --model <model>`
...
`python -m buildkit_cli.registry touch --tool tasks` from the project root. It marks the
