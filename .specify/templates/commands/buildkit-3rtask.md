...
argument-hint: "<task-type: code|plan|strategy|research> <subject> [--manifest <slices.json>] [--builders <n>] [--min-cycles 2] [--max-cycles 5] [--token-budget <n>] [--max-seconds <n>] [--review-only] [--terminal-review auto|on|off] [--confirm-protected] [--allow-secrets] [--accept-single-slice] [--feature <id>]"
compatibility: "Requires a git working tree. The cross-provider Critic needs the `codex` CLI on PATH (authenticated); without it the run still completes with a Claude Critic and a LOUD recorded reduced-independence warning. Fix contexts need a clean tree on a non-protected branch. Catalog touch: ONE additive threerole_run index row per run (migration 0027) + spec-020 token rows — advisory, the fs artifact set under .specify/3rtask/runs/<run_id>/ is the detailed record."
metadata:
  author: "buildkit"
