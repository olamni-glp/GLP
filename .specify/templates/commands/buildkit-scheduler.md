...
argument-hint: "[a natural-language request, or a subcommand: cycle|loop|onboard|plan|board|report|status|doctor|root|version]"
compatibility: "Cross-platform (Windows/Linux/macOS). The scheduling engine is stdlib-only (no third-party runtime dependency) and its system of record is the on-disk file-CRDT substrate under sched_root (R1 indirection: config.local.json `sched_root`, default coop/sched; --root overrides), so it runs fully air-gapped. Optional interoperability only: the shared spec-030 machine-registry PGlite catalog under the deploy home hosts one additive scheduler_cycle metadata row per cycle (auto-bootstraps on first use; on an older catalog run migrations/0029_add_scheduler.py once) and buildkit-co's [co] extra mirrors each event — both degrade to a no-op when absent. Home resolves DB-free (env BUILDKIT_DEPLOY_HOME -> per-machine pointer -> per-user default; --home overrides). Reuses codify.redact for redaction and deploy.home/deploy.session for the catalog."
metadata:
  author: "buildkit"
