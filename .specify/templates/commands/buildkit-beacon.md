...
argument-hint: "[a natural-language goal, or a subcommand: doctor|init|host|web|join|update|list|worker|mcp]"
compatibility: "Windows PoC + WSL for AtomVM. Needs .NET 8 SDK (the host/SPA target net8.0), Python 3.13+, and Node >= 18; the Gleam/AtomVM demonstrator additionally needs WSL with AtomVM v0.6.6 + the Gleam toolchain. Run the beacon's Python with PYTHONPATH including the beacon worktree src. Single freestanding instance (no federation); LAN-trust boundary."
metadata:
  author: "buildkit"
