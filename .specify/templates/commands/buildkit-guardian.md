...
argument-hint: "[a natural-language situation, or a subcommand: ensure-running|start|stop|disable|enable|status|report|module|coverage|findings|resolve|gate|override|backup|config|install-hook]"
compatibility: "Requires a git repository + the buildkit PGlite catalog. Core protection (backups/integrity/gate) runs air-gapped; the optional [co] extra (duckdb/pyzmq) adds the buildkit-co long-text detail mirror. OS-supervisor self-registration uses systemd-user / launchd / Windows Task Scheduler with a session-start ensure backstop on hosts without one."
metadata:
  author: "buildkit"
