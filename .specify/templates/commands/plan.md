...
handoffs: 
  - label: Create Tasks
    agent: buildkit.tasks
    prompt: Break the plan into tasks
    send: true
  - label: Create Checklist
    agent: buildkit.checklist
...
3. **Agent context update**:
   - Update the plan reference between the `<!-- BUILDKIT START -->` and `<!-- BUILDKIT END -->` markers in `__CONTEXT_FILE__` to point to the plan file created in step 1 (the IMPL_PLAN path)
