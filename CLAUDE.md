...
<!-- SPECKIT START -->
Active feature: `002-tutorial-ch01` (Olamni Tutorial — Chapter 1: Fair Stream Merger).

For technologies, dependencies, project structure, shell commands, and other implementation context for the active feature, read the current plan: [`specs/002-tutorial-ch01/plan.md`](specs/002-tutorial-ch01/plan.md).

Companion artefacts under `specs/002-tutorial-ch01/`:
- `spec.md` — feature specification (with 3 resolved Clarifications: locked binding `[1, a, 2, b, 3]`; status-block approval-gate format; learner-targeted trace structure)
- `research.md` — Phase 0 decisions (R-001 comment density, R-002 REPL build location + gitignore, R-003 top-level tutorial.md structure, R-004 inspection goals, R-005 Dart verification, R-006 PDF re-read scope)
- `data-model.md` — entities (Exercise, ChapterTutorial, TopLevelIndex, ApprovalGate) with state transitions and lifecycle
- `contracts/trace-file-format.md` — `ex-NN-repl-trace.md` structural contract
- `contracts/status-block-format.md` — `chXX_tutorial.md` approval-gate format contract
- `contracts/glp-file-format.md` — `.glp` content + paraphrase-comment + variable-naming contract
- `quickstart.md` — implementer's quickstart (the LEARNER-facing quickstart will live in `olamni/tutorial/ch01/exercise-01/ex-01-tutorial.md` post-implement)
- `checklists/requirements.md` — quality checklist from `/speckit-clarify`
- `tasks.md` — Phase 2 task list (when generated by `/speckit-tasks`)

Constitution governing this feature: [`.specify/memory/constitution.md`](.specify/memory/constitution.md) v1.2.0.

Workflow memory for replicating this on ch02–ch13: [`memory/olamni_tutorial_chapter_workflow.md`](memory/olamni_tutorial_chapter_workflow.md) (in the Claude memory dir).
<!-- SPECKIT END -->
