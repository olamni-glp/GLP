...
<!-- SPECKIT START -->
Active feature: `007-tutorial-ch06` (Olamni Tutorial — Chapter 6: Typed Programming).

For technologies, dependencies, project structure, shell commands, and other implementation context for the active feature, read the current plan: [`specs/007-tutorial-ch06/plan.md`](specs/007-tutorial-ch06/plan.md).

Companion artefacts under `specs/007-tutorial-ch06/`:
- `spec.md` — feature specification (with 2 resolved Clarifications: Q1 ex-01 source = ch04 §4.3.7 `flatten`+`flatten_acc` per option B + ex-03 source = ch04 §4.4.4 control MI per input-prompt default; Q2 type/procedure declaration shapes deferred to /speckit-plan T006-equivalent per ch05 Q2 precedent).
- `research.md` — Phase 0 decisions (R-001 ~25–35 %% comments across 5 .glp files, R-002 REPL build inherited from ch05 commit `bcd59392`, R-003 top-level update + ch06 footnote per FR-014, R-004 inspection goals deferred to /speckit-implement T006, R-005 Dart 3.10.1, R-006 type-checker pre-flight inherited from ch05 R-006, R-007 declaration-shape sketches per Q2 deferral with locked-shape table to be appended at T006, R-008 cross-chapter relationship contract NEW for ch06 = synthesis-from-earlier-chapters distinct from ch04 inversion + ch05 typed↔untyped + ch02 forward-import, R-009 locked filenames matching §6.x heading text directly).
- `data-model.md` — entities (Exercise ×5 with §6.x heading + source map, Source Program ×5 from ch03/ch04/ch05, Approval Gate ×4 pairwise, Cross-chapter Relationship of type synthesis-from-earlier-chapters NEW for ch06, Chapter Tutorial, Top-level Index with ch06 footnote NEW) with state transitions.
- `contracts/trace-file-format.md` — 5 phases per trace (1 load + primary + 3 inspection); strict byte-equality; no negative-exercise / per-run-variation contracts (ch06 has no negatives, no `now/1`/`'_output'/1`).
- `contracts/status-block-format.md` — per-exercise 5-line block; pairwise-gate grep contract (`grep -E "^- exercise-0NN: approved [0-9]{4}-[0-9]{2}-[0-9]{2}"` returns 1 before ex-(NN+1) work). Pairwise inherits from ch01–ch03 NOT ch04/ch05 group-gate.
- `contracts/glp-file-format.md` — 5 file specs (one .glp each, no two-file pattern); synthesis cross-reference header block NEW for ch06 per R-008; ex-02 exception (declarations byte-exact from ch05 §5.6 including ch05 Q10 amendments); per-clause %% paraphrase comments from charter §1.5.
- `quickstart.md` — implementer's sequential implementation order (5 exercises pairwise-gated; ~53 numbered steps); pre-flight checklist + R-006 type-checker verification + ch06 PDF stub state re-verification per FR-015.
- `checklists/requirements.md` — quality checklist from `/speckit-specify` (all items pass; both clarifications resolved per /speckit-clarify session 2026-05-01).
- `tasks.md` — Phase 2 task list (generated by `/speckit-tasks`).

Predecessor (ch01 + ch02 + ch03 + ch04 + ch05) artefacts at `specs/002-tutorial-ch01/` through `specs/006-tutorial-ch05/` are inherited as the model. Branch number → dir number mapping per workflow memory: ch01→002, ch02→003, ch03→004, ch04→005, ch05→006, ch06→007, …, ch13→014.

Constitution governing this feature: [`.specify/memory/constitution.md`](.specify/memory/constitution.md) v1.2.0.

Workflow memory for replicating this on ch07–ch13: [`memory/olamni_tutorial_chapter_workflow.md`](memory/olamni_tutorial_chapter_workflow.md) (in the Claude memory dir).
<!-- SPECKIT END -->
